...
async def rename_handler(bot: Client, event: Message):
    ...
    if (Config.ONE_PROCESS_ONLY is False) and (isInGap is True):
        await event.reply_text(f"**Please Wait...⏳Till** `{str(t_)}s`", quote=True)
        return
        ...
        try:
            ...
            if ask_.text and (ask_.text.startswith("/") is False):
                ...
                if len(new_file_name) > 255:
                    await reply_.edit("**File Name Length Is More Than 255 bytes , Send Blew 255 Bytes**")
                    return
                await ask_.delete(True)
                await reply_.edit("**Downloading ⬇️**")
                ...
                try:
                    ...
                    if not os.path.lexists(new_file_name):
                        try:
                            await reply_.edit("**Something Went Wrong , Unable To Find File**")
                        except:
                            print(f"**Something Went Wrong , Unable To Find File For {str(event.from_user.id)}**")
                        return
                    await asyncio.sleep(Config.SLEEP_TIME)
                    await reply_.edit("**Uploading ⬆️**")
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "closeMeh" in cb.data:
        ...
        if thumbnail is None:
            ...
            await cb.answer("Thumbnail Found", show_alert=True)
            try:
                await bot.send_photo(
        ...
        await cb.answer("Removed Custom Thumbnail Succesfully", show_alert=True)
        await OpenSettings(cb.message, user_id=cb.from_user.id)
